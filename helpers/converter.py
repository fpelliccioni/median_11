...
def get_return(fn_name, fn_args, part):
    if fn_name != None:
        ...
        for p in tmpargs:
            ...
            actual_args += f'std::forward<{p.upper()}>({p}), '
        ...
        tmp_actual_args = tmp_actual_args[:len(tmp_actual_args) - 2]

        return f'return **{fn_name}**<{tmp_actual_args}>({actual_args}r)'
    else:
        return f'return std::forward<{part.upper()}>({part})'
...
def process(n,i,cost,comps,fn_name,fn_args,var1,var2,if_part,else_part, functions_aliases):
    ...
    parameters = ''
    for p in args:
        template_parameters += f'int {p}i, '
        parameters += f'{p.upper()}&& {p}, '
    for p in args:
        template_parameters += f'Regular {p.upper()}, '

    requires = ''
    for p in args[1:]:
        requires += f'Same<A, {p.upper()}> && '
    ...
    code  = f'// {fn_name} ******************* \n'
    code += f'template <{template_parameters}StrictWeakOrdering R>\n'
    code += f'    requires({requires}Domain<R, A>)\n'
    code += f'inline constexpr\n'
    code += f'auto&& {new_func_name}({parameters}R r) noexcept ' + '{\n'
    code += f'    compare_strict_or_reflexive<({var1}i < {var2}i), R> cmp;\n'
    code += f'    if (cmp({var1}, {var2}, r)) {return_if};\n'
    code += f'    {return_else};\n'
    code += '}\n\n'
